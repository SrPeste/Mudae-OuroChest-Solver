...
class SolverState:
    ...
    def recommendation_reason(self, pos: Pos) -> str:
        ...
        if p_red >= 1.0:
            return f"Vermelho confirmado em ({pos[0]},{pos[1]})."
        if p_red > 0:
            return (
                f"Clique ({pos[0]},{pos[1]}): {p_red:.0%} chance de vermelho, "
                f"~{expected:.0f} SP esperado, {possible} reds possíveis."
            )
        return (
            f"Clique ({pos[0]},{pos[1]}): ~{expected:.0f} SP esperado, "
            f"reduz candidatos ({possible} reds restantes)."
        )
