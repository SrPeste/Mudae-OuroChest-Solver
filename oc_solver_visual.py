...
class OcSolverApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        self.lbl_reds = tk.Label(
            right,
            text=f"Reds possible: 24/{TOTAL_RED_POSITIONS}",
            font=("Segoe UI", 10),
            fg="#ccc",
            bg="#1a1a2e",
        )
    ...
    def _bot_execute_click(self, pos: Pos) -> None:
        ...
        try:
            ...
            self.log(
                f"   Click in ({pos[0]},{pos[1]}) → "
                f"{color.label_pt} (+{color.value_sp} SP)"
            )
            if color == Color.RED:
                self.log("   ★ Red founded! +150 SP")
            self.refresh_all()
            if self.session.found_red:
                self.log(f"   Match won! Total: {self.session.score} SP")
                self.bot_running = False
                self.btn_bot.configure(text="▶ Bot resolver (its only an example)")
                return
            if self.session.remaining_clicks() <= 0:
                self.log(
                    f"   No clicks left. Total: {self.session.score} SP"
                )
                self.bot_running = False
                self.btn_bot.configure(text="▶ Bot resolver (its only an example)")
                return
    ...
    def refresh_all(self, manual: bool = False, show_hidden: bool = False) -> None:
        ...
        self.lbl_score.configure(
            text=f"Points: {score} SP"
        )

        self.lbl_clicks.configure(
            text=f"Clicks: {clicks_used} / {self.session.max_clicks}"
        )

        self.lbl_reds.configure(
            text=f"Reds possible: {len(possible_reds)}/{TOTAL_RED_POSITIONS}"
        )

        # Atualiza status
        if manual:
            if not valid:
                status = "⚠ Invalid Config — check your mudae $oc colors."
                color = "#e94560"
            else:
            ...
            status = "Red Founded! Best results."
            color = "#4ecca3"

        elif clicks_used >= self.session.max_clicks:
            status = f"End {self.session.max_clicks} clicks — {score} SP total."
            color = "#ffd369"
            ...
            status = "Click ▶ Bot resolver or define the colors manually."
            color = "#888"
